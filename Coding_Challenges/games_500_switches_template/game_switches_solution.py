...
def switch(oracle):
    ...
    def circuit():
        ...
        # initialize first three qubits in |+> state
        for i in range(3):
            qml.Hadamard(wires=[i])
        # initialize light qubit in |-> state
        qml.PauliX(wires="light")
        qml.Hadamard(wires="light")

        oracle()

        # undo Hadamard to return to computational basis
        for i in range(3):
           qml.Hadamard(wires=[i])
    ...
    # if qubit == 1 in computational basis, light is "on"
    list = []
    for i in range(3):
        if sample[i] == 1:
            list.append(i)
